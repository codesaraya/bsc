      ...
      <footer className="relative bg-gradient-to-b from-[#0d0b2e] via-[#1A1464] to-[#0d0b2e] text-white pt-12 md:pt-20 pb-8 overflow-hidden">
        ...
        <Container className="relative z-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-12 pb-8 md:pb-12 border-b border-white/10">
            ...
            {/* Locations */}
            <div>
              <h4 className="font-bold text-lg mb-6">{data?.contactTitle || "Naše lokacije"}</h4>
              <div className="space-y-4">
                {locations.map((loc: any, i: number) => (
                  <div key={i} className="flex items-start gap-2 text-gray-400 text-[13px]">
              ...
              <div className="mt-5 space-y-2">
                <div className="flex items-center gap-2 text-gray-400 text-[13px]">
                  ...
                  <a href={`tel:${locations[0]?.phone || '+387 33 571 111'}`} className="hover:text-primary transition-colors">{locations[0]?.phone || '+387 33 571 111'}</a>
                </div>
                <div className="flex items-center gap-2 text-gray-400 text-[13px]">
                  <Mail className="w-3.5 h-3.5 text-primary shrink-0" />
                  <a href={`mailto:${locations[0]?.email || 'bscsarajevo@gmail.com'}`} className="hover:text-primary transition-colors">{locations[0]?.email || 'bscsarajevo@gmail.com'}</a>
                </div>
              </div>
            </div>
          </div>
