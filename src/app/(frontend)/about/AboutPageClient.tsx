...
const equipmentIconMap: Record<string, LucideIcon> = {
  Printer, Zap, Scissors, Target, Sparkles, Star, Shield, Award,
  Cog: Settings, Cpu, Layers, Box,
};

const defaultEquipment = [
  {
    name: "AEON Laser",
    category: "Lasersko rezanje i graviranje",
    description: "AEON Laser su industrijski CO2 laserski sistemi za precizno rezanje i graviranje širokog spektra materijala — akril, drvo, MDF, koža, staklo, papir, tekstil i mnogi drugi. Koristimo ih za izradu reklamnih panela, svetlećih reklama, pleksiglas stalaka, gravura, dekorativnih elemenata i personaliziranih proizvoda. Laserska preciznost omogućava složene oblike i fine detalje koje tradicionalne metode ne mogu postići.",
    icon: Zap,
  },
  {
    name: "Gong Zheng UV",
    category: "UV flatbed štampa",
    description: "Gong Zheng UV flatbed printeri koriste najnoviju UV LED tehnologiju sa Ricoh Gen6 i Epson I3200 glavama za direktan print na ravne i krute materijale. Štampamo na staklu, pleksiglasu, alubondu, foreksu, MDF-u, drvetu, keramici i još mnogo toga. Naša mašina velikog formata sa širinom štampe od 3.2m jedina je takva u regiji, što nam daje apsolutnu prednost u kvalitetu i kapacitetu UV štampe.",
    icon: Printer,
  },
  {
    name: "HP Latex",
    category: "Latex štampa velikog formata",
    description: "HP Latex printeri koriste vodenu latex tehnologiju tinte koja je ekološki prihvatljiva, bez mirisa i sigurna za unutrašnju upotrebu. Idealni su za postere, bannere, vehicle wrapping, tapete, POS materijale i dekorativne grafike. Otisci su odmah suhi, otporni na ogrebotine i UV zračenje, s živim bojama koje traju godinama — savršeni za unutrašnju i vanjsku primjenu.",
    icon: Sparkles,
  },
  {
    name: "MUTOH",
    category: "Eco-solvent štampa",
    description: "MUTOH eco-solvent printeri (ValueJet serija) specijalizirani su za visokokvalitetnu štampu na vinil folijama, bannerima, ceradama, naljepnicama i raznim fleksibilnim materijalima. Eco-solvent tinte pružaju izuzetnu otpornost na vremenske uvjete, UV zračenje i vlagu, čineći ih idealnim za outdoor reklame, brendiranje vozila, izloge i dugotrajne grafičke aplikacije.",
    icon: Printer,
  },
  {
    name: "SUMA CNC",
    category: "CNC rezanje i plotanje",
    description: "SUMA CNC mašine služe za precizno konturno rezanje vinila, folija, reflektirajućih materijala i tanjih fleksibilnih podloga. Koriste se za izradu naljepnica, oznaka, sigurnosnih traka, brendiranja vozila i dekorativnih grafika. Automatsko pozicioniranje i visoka preciznost osiguravaju čiste rezove čak i kod najsloženijih oblika i sitnih detalja.",
    icon: Scissors,
  },
  {
    name: "Dizart CNC",
    category: "CNC glodanje i rezanje",
    description: "Dizart CNC mašine su industrijski CNC routeri za glodanje i rezanje krutih materijala — alubond, PVC, akril, drvo, MDF, forex i kompozitne ploče. Koristimo ih za izradu 3D slova, reklamnih totema, svjetlećih kutija, dekorativnih panela i složenih prostornih konstrukcija. Preciznost CNC obrade osigurava savršene rubove i ponavljivi kvalitet u serijskoj proizvodnji.",
    icon: Cpu,
  },
  {
    name: "XEROX",
    category: "Digitalna produkcijska štampa",
    description: "XEROX digitalni produkcijski printeri omogućavaju brzu i kvalitetnu štampu na papirnim medijima — letci, brošure, katalozi, vizitke, plakati i razni promo materijali. Napredna tehnologija boja osigurava konzistentne, žive boje uz oštru reprodukciju teksta i slika. Idealni su za manje i srednje naklade gdje brzina i kvalitet moraju ići ruku pod ruku.",
    icon: Layers,
  },
  {
    name: "CANON",
    category: "Digitalna produkcijska štampa",
    description: "CANON digitalni printeri dopunjuju naš produkcijski kapacitet i pružaju izuzetan kvalitet štampe za kataloge, knjige, časopise, poslovnu dokumentaciju i visokokvalitetne promo materijale. Njihova pouzdanost i precizna reprodukcija boja čine ih nezamjenjivim za projekte koji zahtijevaju najviše standarde kvaliteta u digitalnoj štampi manjih i srednjih formata.",
    icon: Star,
  },
];
...
export default function AboutPageClient({ data, clientsData }: AboutPageClientProps) {
  ...
  const equipment =
    d?.equipmentItems?.length > 0
      ? d.equipmentItems.map((e: any, i: number) => ({
          name: e.name,
          category: e.category || '',
          description: e.description,
          icon: (e.icon && equipmentIconMap[e.icon]) || defaultEquipment[i]?.icon || Printer,
        }))
      : defaultEquipment;
  ...
  return (
    <>
          ...
          <div className="grid md:grid-cols-2 gap-10 md:gap-16 items-center">
            ...
            <motion.div {...fadeUp} transition={{ duration: 0.5, delay: 0.2 }}>
              ...
              <p className="text-gray-500 leading-relaxed mb-4 max-w-md">
                {d?.description1 ||
                  "Best Solution Company je specijalizirana štamparija opremljena najnovijim digitalnim UV mašinama, uključujući UV LED mašinu velikog formata sa širinom štampe od 3.2m — jedinu mašinu u ovoj regiji — kao i Gong Zheng UV koja štampa sve ravne materijale (staklo, pleksiglas, alubond, forex, MDF, drvo itd.)"}
              </p>
      ...
      <section className="py-12 md:py-20">
        <Container>
          <SectionTitle
            badge={d?.equipmentBadge || "Naša oprema"}
            title={d?.equipmentTitle || "Mašine koje pokreću naš rad"}
            subtitle={d?.equipmentSubtitle || "Koristimo vodeće svjetske brendove u digitalnoj štampi, laserskom rezanju i CNC obradi kako bismo vam pružili vrhunski kvalitet."}
          />
