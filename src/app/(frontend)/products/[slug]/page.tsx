...
const categoryIcons: Record<string, React.ReactNode> = {
  brendiranje: <Megaphone className="w-10 h-10" />,
  "outdoor-indoor": <Sun className="w-10 h-10" />,
  "home-and-office": <Home className="w-10 h-10" />,
  "promo-pos": <ShoppingBag className="w-10 h-10" />,
};
...
export default async function ProductCategoryPage({
  ...
  if (!category) {
    category = getStaticProductCategoryBySlug(slug);
  }
  ...
  const defaultWhyCards = [
    { title: "Premium kvalitet", text: "Koristimo najmodernije tehnologije štampe i obradu vrhunskim materijalima" },
    { title: "Garancija kvaliteta", text: "Svaki proizvod prolazi rigoroznu kontrolu kvaliteta prije isporuke" },
    { title: "Brza isporuka", text: "Efikasan proces od upita do gotovog proizvoda — čak i za hitne narudžbe" },
    { title: "Prilagodljivo", text: "Potpuno prilagođeno vašim potrebama — dimenzije, materijali, oblici" },
    { title: "Stručni tim", text: "Naši dizajneri i tehničari imaju višegodišnje iskustvo u industriji" },
    { title: "Brojni zadovoljni klijenti", text: "Stotine realizovanih projekata za domaće i međunarodne klijente" },
  ];
        ...
        <Container className="relative z-10">
          ...
          <div className="flex items-center gap-6 mt-10 flex-wrap">
            <div className="px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/10">
              ...
              <span className="text-white/60 ml-1 text-sm">{cp?.productsCountLabel || 'proizvoda'}</span>
            </div>
            <div className="px-5 py-2.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/10">
        ...
        <Container>
          <div className="max-w-5xl mx-auto">
            <h2 className="text-3xl font-bold text-dark mb-2">
              {cp?.availableTitle || 'Dostupni Proizvodi'}
            </h2>
            <p className="text-gray-500 mb-10">
              {cp?.availableSubtitle || 'Izaberite proizvod za detaljne informacije, galeriju i mogućnosti prilagodbe.'}
            </p>
              ...
              <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="text-white">
                  <h3 className="text-2xl md:text-3xl font-bold mb-2">
                    {cp?.ctaPrefix || 'Zainteresirani ste za'} {category.title.toLowerCase()}?
                  </h3>
                  <p className="text-white/60 max-w-lg">
                    {cp?.ctaDescription || 'Kontaktirajte nas za besplatnu ponudu i profesionalan savjet za vaš projekat.'}
                  </p>
                </div>
                  ...
                  >
                    ...
                    <div className="relative z-10">
                      ...
                      <span className="inline-flex items-center gap-1 text-xs text-white/40 mt-3">
                        {cat.items.length} {cp?.productsCountLabel || 'proizvoda'} <ArrowRight className="w-3 h-3" />
                      </span>
                    </div>
                  </Link>
                ))}
          ...
          <div className="text-center mt-8">
            ...
            >
              <ArrowLeft className="w-4 h-4" />
              {cp?.backToAllText || 'Nazad na sve proizvode'}
            </Link>
          </div>
        </Container>
      </section>
